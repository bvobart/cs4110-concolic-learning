...
public class BasicLearner {
	...
	public static void runSimpleExperiment(
		...
		experiment.run();
		PrintWriter log = learningSetup.getLogWriter();
		log(log, "Ran " + experiment.getRounds().getCount() + " rounds");
		log.close();
		produceOutput(learningSetup, FINAL_MODEL_FILENAME, experiment.getFinalHypothesis(), alphabet, true);
	}
	...
	public static void runSimpleExperiment (
		...
		runSimpleExperiment(learningSetup, learningSetup.learner, learningSetup.eqOracle, learlibAlphabet);
	}
	 ...
	 * @param learningSetup The learning setup
	 * @param alphabet Input alphabet
	 * @throws IOException
	 */
	public static void runControlledExperiment(
			LearningSetup learningSetup,
			Alphabet<String> alphabet) throws IOException {
		LearningAlgorithm<MealyMachine<?, String, ?, String>, String, Word<String>> learner = learningSetup.learner;
		EquivalenceOracle<MealyMachine<?, String, ?, String>, String, Word<String>> eqOracle = learningSetup.eqOracle;
		Counter nrSymbols = learningSetup.nrSymbols;
		Counter nrResets = learningSetup.nrResets;
		PrintWriter logWriter = learningSetup.getLogWriter();
		try {
			...
			while(true) {
				...
				if(saveAllHypotheses) {
					...
					produceOutput(learningSetup, outputFilename, learner.getHypothesisModel(), alphabet, false);
					log(logWriter, "model size " + learner.getHypothesisModel().getStates().size());
				}
	
				// Print statistics
				log(logWriter, stage + ": " + Calendar.getInstance().getTime());
				// Log number of queries/symbols
				log(logWriter, "Hypothesis size: " + learner.getHypothesisModel().size() + " states");
				long roundResets = nrResets.getCount() - lastNrResetsValue, roundSymbols = nrSymbols.getCount() - lastNrSymbolsValue;
				log(logWriter, "learning queries/symbols: " + nrResets.getCount() + "/" + nrSymbols.getCount()
				...
				log(logWriter, "testing queries/symbols: " + nrResets.getCount() + "/" + nrSymbols.getCount()
				...
				if(ce == null) {
					...
					log(logWriter, "\nFinished learning!");
					produceOutput(learningSetup, FINAL_MODEL_FILENAME, learner.getHypothesisModel(), alphabet, true);
					break;
				} else {
			...
			produceOutput(learningSetup, errorHypName, learner.getHypothesisModel(), alphabet, true);
			throw e;
		} finally {
			logWriter.close();
		}
	...
	public static void runControlledExperiment(
		...
		runControlledExperiment(learningSetup, learnlibAlphabet);
	}
	...
	public static void runControlledExperiment(
		...
		Alphabet<String> learnlibAlphabet = new SimpleAlphabet<String>(alphabet);
		LearningSetup learningSetup = new LearningSetup(problem, learningMethod, testingMethod, learnlibAlphabet);
		runControlledExperiment(learningSetup, learnlibAlphabet);
	}
	 ...
	 * @throws IOException
	 */
	public static void produceOutput(LearningSetup learningSetup, String fileName, MealyMachine<?,String,?,String> model, Alphabet<String> alphabet, boolean verboseError) throws FileNotFoundException, IOException {
		fileName = learningSetup.getRelativeResultsPath() + fileName;
	...
	public static void log(PrintWriter pw) {
		log(pw, "");
	}

	public static void log(PrintWriter pw, String line) {
	...
	public static class LearningSetup {
		public EquivalenceOracle<MealyMachine<?, String, ?, String>, String, Word<String>> eqOracle;
		public LearningAlgorithm<MealyMachine<?, String, ?, String>, String, Word<String>> learner;
		public Counter nrSymbols, nrResets;
		public Problem problem;
		public LearningMethod learningMethod;
		public TestingMethod testingMethod;
		...
		public LearningSetup(SUL<String,String> sul, LearningMethod learningMethod, TestingMethod testingMethod, Alphabet<String> alphabet) {
			construct(sul, learningMethod, testingMethod, alphabet);
		}

		public LearningSetup(Problem problem, LearningMethod learningMethod, TestingMethod testingMethod, Alphabet<String> alphabet) {
			this.problem = problem;
			construct(problem.getSUL(), learningMethod, testingMethod, alphabet);
		}

		public void construct(SUL<String,String> sul, LearningMethod learningMethod, TestingMethod testingMethod, Alphabet<String> alphabet) {
		...
		public void makeResultsDirectory() {
			new File(getRelativeResultsPath()).mkdirs();
		}

		public String getRelativeResultsPath() {
			return problem.getRelativeResultsPath(learningMethod, testingMethod);
		}

		public PrintWriter getLogWriter() throws IOException {
			return problem.getLogWriter(learningMethod, testingMethod);
		}
	}
}
